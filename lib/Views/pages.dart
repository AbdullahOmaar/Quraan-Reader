...
class _MyHomePageState extends State<MyHomePage> {
  ...
  List<Widget> _listOfImages() {
    List<Widget>ImageCards=[];
        for ( int i=0; i<images.length; i++ ) ImageCards.add(
  ...
  void _printTap(String gesture, TapPosition position) =>
      print('$gesture: ${position.global}');

   yalla(TapPosition position) {
    ...
    {
      var coords = _ayasPage[i].ayaCoordinates!.split(',');
      // print("-------------------");
      // print(coords.length);
      // print(coords);
      // print("-----------------");
      ...
      for (int j=0;j<coords.length-1;j=j+4)
      {
        ...
        // Check on X
        if (position.global.dx.round()>=(int.parse(coords[j])*heightR) && position.global.dx.round()<=(int.parse(coords[j+2])*heightR))
        {
          //Check on Y
          if (position.global.dy.round()>=(int.parse(coords[j+1])*widthR) && position.global.dy.round()<=(int.parse(coords[j+3])*widthR))
          {

            print(int.parse(coords[j+1])*widthR);
...
print("------------------------------------------------");
  ...
  async {
    ...
    var out2=await DBHelper.instance.readAllAyaInPage(widget.ind+1);
